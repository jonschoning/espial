FROM fpco/stack-build:lts-24.37 AS builder-base
...
RUN mkdir -p /tmp/buildinfo && \
  echo "module BuildInfo where" > /tmp/buildinfo/BuildInfo.hs && \
...
RUN apt-get update \
  ...
  && wget -O /usr/local/bin/monolith "https://github.com/Y2Z/monolith/releases/download/${MONOLITH_VERSION}/monolith-gnu-linux-x86_64" \
  && chmod +x /usr/local/bin/monolith \
  && rm -rf /var/lib/apt/lists/*

# Variant image for ARCHIVE_BACKEND=singlefile, which needs node alongside espial for
# single-file-cli; distroless can't host it. Connects to a chromium CDP endpoint running in a
# separate container (SINGLEFILE_BROWSER_SERVER) -- no chromium is bundled in this image.
# Build with `--target runtime-singlefile`.
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends ca-certificates libgmp10 nodejs npm zlib1g \
